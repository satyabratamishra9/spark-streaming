...
object AppLaunch {
  ...
  def consumeFromKafka(topic: String): Unit = {
    ...
    val messages = KafkaUtils.createDirectStream[String, String](ssc,
      ...
      ConsumerStrategies.Subscribe[String, String](Seq(topic), consumer))
    messages.map(record => {
    ...
    //      })
      ...
      def insertToMongo(record: Document): Unit = {
        consumer.commitSync()
...
//        val doc: Document = BsonDocument(s1)
        ...
        val insertObservable = tweet.insertOne(record)
